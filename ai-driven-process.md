...
| 设计 | 人设计、人评审 | AI 生成架构/数据库/接口设计 + AI 自审 | ⭕ 可选：默认信任 AI 自审 |
...
> 流程中只有 3 个菱形判断需要人：**确认 SRS**（必须）、**抽查设计**（可选）、**最终验收**（必须）。其余全部自动循环。
...
- **自检**：逐条对照需求跟踪矩阵检查覆盖；遵循"满足需求前提下尽量简单"防止过度设计
...
### 4.3 为什么要"最大迭代轮次"熔断
测试不过 → 修复 → 重测的循环可能不收敛（AI 在两个错误方案之间摇摆）。设置最大轮次（建议 3~5 轮），超限即升级给人，并附上全部迭代历史供定位。

### 4.4 人保留的两个节点的意义
- **确认 SRS**：这是用户意图进入系统的唯一入口。SRS 错，后面 AI 执行得再完美也是南辕北辙。所以这个检查点不能省，且 SRS 必须是"人能看懂的自然语言 + 原型"。
