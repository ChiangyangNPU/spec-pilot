...
- [x] 与设计的差异全部记录在差异表（明确写"与设计一致"），无静默分叉
- [x] 单元测试覆盖正常/边界/异常三类场景，且测试方法名/docstring 携带来源用例 ID
- [x] 本地运行指南按文档真实执行过一次，步骤可复现
- [x] 实现未超出需求范围（无顺手加戏的功能）
- [x] 构建、单元测试真实运行且全绿（退出码为准）；Lint 未执行的原因已在测试报告注明
