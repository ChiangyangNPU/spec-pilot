...
| `docs/01-srs.md` | 阶段 1 | 每条功能需求附带可自动验证的验收标准（如"退出码 2"而非"报错"） |
| `docs/02-design.md` | 阶段 2 | 模块单向依赖、退出码约定、需求覆盖对照表 |
| `docs/03-tasks.md` | 拆解 | 每个任务的完成判据都能对应到测试用例 ID |
| `docs/04-test-cases.md` | 编码前冻结 | 30 个用例只从 SRS/设计反向生成，未看实现；需求覆盖矩阵无缺口 |
