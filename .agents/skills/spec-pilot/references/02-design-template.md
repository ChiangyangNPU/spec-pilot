...
> 无数据库的项目（如文件存储、纯内存工具）不强行画 E-R 图：以"持久化格式设计"代替——数据文件结构、字段约定、损坏时的行为（报错而非清空）写清楚即可，仍然必须回答"数据长什么样、坏了好不好恢复"。
>
> **涉及既有数据结构变更时**，必须写明迁移策略：迁移脚本/命令、新旧格式兼容方式（能否双读/灰度）、迁移失败或需回退时的处理——存量用户的数据不是测试数据，不能"上线后见"；确实不迁移（旧数据保持原样可读）也要作为显式决策写出来，而不是遗漏。
